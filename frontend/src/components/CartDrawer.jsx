  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-slate-950/80 backdrop-blur-md transition-opacity">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-0 sm:pl-10">
        <div className="w-full sm:w-[420px] max-w-full bg-slate-900 border-l border-slate-800 text-white shadow-2xl flex flex-col justify-between">
          
          {/* Drawer Header */}
          <div className="p-4 sm:p-6 border-b border-slate-800 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <ShoppingBag className="w-5 h-5 text-amber-400" />
              <h2 className="text-base sm:text-lg font-bold font-display text-white">Your Guest Cart</h2>
          ...
          {cartItems.length > 0 && (
            <div className="p-4 sm:p-6 border-t border-slate-800 space-y-3 sm:space-y-4 bg-slate-950/80">
              ...
              <div className="p-2.5 sm:p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-center gap-2 text-[10px] sm:text-[11px] text-amber-300">
                <Lock className="w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0 text-amber-400" />
                <span>Login or Account Sign In required at checkout to complete order placement.</span>
              </div>

              <button
                onClick={onProceedToCheckout}
                className="w-full min-h-[48px] flex items-center justify-center gap-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 py-3 sm:py-4 rounded-xl font-extrabold text-xs sm:text-sm shadow-xl shadow-amber-500/20 transition-all cursor-pointer"
              >
